...
class Test_CreateOrder:
    ...
    @pytest.fixture()
    def test_CreateOrder(self,login):
        ...
        url = test_ip + '/platform/oms/order/createOrder'
        response = requests.post(url,json = payloads, headers = headers, token = login().token).json()
        sellorderId = jsonpath.jsonpath(response, "$..sellOrderId")
